<?php
    ...
    if(isset($_POST['Submit']))
    {
        if(validate())
        {
            ...
            if(mysqli_query($conn,$sql))
            {
                ...
                if ($result = mysqli_query($conn, $sqlt)) 
                {
                    ...
                    if($rowcount==0)
                    {
                        ...
                        if ($sqls) 
                        {
                            echo "<script>alert('Student Details insertion successful.');location.href='Q5.html';</script>";
                        } 
                        else 
                        {
                            echo "<script>alert('Error uploading details, try again after some time.');location.href='Q5.html';</script>";
                                exit();
                        }
                    }
                    else
                    {
                        echo "<script>alert('Details already available with this Roll Number.');location.href='Q5.html';</script>";
                     exit();
                    }
                }
                else
            ...
            echo "<script>location.href='Q5.html';</script>";
    }
    $conn->close();
?>